class Llvm < Formula
  ...
  on_macos do
    # Apple Clang 12.x (Xcode 12 / macOS 10.15) has a compiler bug where
    # *consuming* (not building) a precompiled header that contains
    # llvm::bit_cast<>() -- lib/Support's `inline` function template whose
    # SFINAE constraints are expressed as defaulted `std::enable_if_t<>` type
    # parameters -- produces a false "non-void function 'bit_cast' should
    # return a value" hard error [-Wreturn-type], not merely a warning
    # promoted by -Werror.
    #
    # Confirmed in isolation: emitting the PCH from a header containing this
    # exact bit_cast pattern succeeds under Apple Clang 12; only a *second*
    # translation unit that includes the built PCH and instantiates a caller
    # (e.g. llvm::APInt::bitsToDouble()/bitsToFloat(), exactly as ABIBreak.cpp
    # and friends do via Support/pch.h) fails identically to the real build:
    #   llvm/include/llvm/ADT/bit.h:92:3: error: non-void function 'bit_cast'
    #   should return a value [-Wreturn-type]
    # This first breaks llvm/lib/Support/{ABIBreak,AMDGPUMetadata,
    # APFixedPoint,APFloat}.cpp.o and would cascade through the rest of the
    # ~9,135-target build. llvm@22's own clang does not reproduce this bug
    # against the same isolated repro, so host-compile with it instead of
    # system Apple Clang.
    #
    # This is a host-compiler bug, not a source defect: the resulting
    # binaries' target codegen/ABI is unaffected (LLVM's own build is fully
    # self-consistent since ENV.llvm_clang applies uniformly to the whole
    # build; bit_cast itself is header-only `inline` and never exported from
    # libLLVM.dylib, so no cross-compiler symbol-mangling exposure there).
    # The one real consumer that links directly against LLVM/Clang's C++ API
    # (include-what-you-use) is pinned to llvm@22 explicitly in this tap for
    # its own, independent compatibility reasons -- see include-what-you-use.rb.
    #
    # Remove this once Apple Clang on a supported OS no longer has this PCH
    # bug, or homebrew-core ships a Catalina bottle.
  ...
  test do
    ...
    unless versioned_formula?
      ...
      # Upstream test bug (not macOS 10.15-specific): this assertion always
      # invokes the Z3 constraint manager regardless of whether Z3 support
      # was actually compiled in. `install` only enables Z3 on Linux or
      # macOS Sonoma+ (LLVM_ENABLE_Z3_SOLVER=OFF here, since Z3 needs
      # std::format from Xcode 15.3+), so unconditionally running this on
      # any source build for older macOS fails with:
      #   "analyzer constraint manager 'unsupported-z3' is only available
      #   if LLVM was built with -DLLVM_ENABLE_Z3_SOLVER=ON"
      # This would affect any pre-Sonoma macOS attempting a --build-from-source
      # install of this formula, not just Catalina. Guard with the same
      # deps-based check `install` uses to decide `enable_z3`.
      if deps.map(&:name).include?("z3")
