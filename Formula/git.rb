class Git < Formula
  ...
  # Upstream removed libiconv on modern macOS (homebrew-core/pull/258461).
  # On macOS 10.15 system libiconv is too old; keep the Homebrew dep and
  # set ICONVDIR explicitly so git links against it.
  on_macos do
    depends_on "gettext"
    depends_on "libiconv"
  end
  ...
  def install
    ...
    # `make test` is omitted: the test harness sources git's t/test-lib.sh,
    # which requires infrastructure not available in the Homebrew build
    # sandbox on macOS 10.15. `make build` generates the script without
    # running tests.
  ...
  test do
    ...
    # git send-email requires Perl >= 5.26.0 (Git/LoadCPAN.pm minimum).
    # macOS 10.15 ships Perl 5.18.4; skip this portion on Catalina.
